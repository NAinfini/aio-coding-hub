...
use crate::gateway::oauth::provider_trait::OAuthTokenSet;
...
async fn parse_token_response(response: reqwest::Response) -> AppResult<OAuthTokenSet> {
    ...
    if !status.is_success() {
        ...
        {
            return Err(
                "AUTH_RELOGIN_REQUIRED: oauth refresh token was already rotated/reused; please use \u{6d4f}\u{89c8}\u{5668}\u{767b}\u{5f55} to reauthorize this account"
                    .to_string()
                    .into(),
            );
        }
