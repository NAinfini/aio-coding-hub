...
"#,
            ...
            return get_summary_by_id(conn, id);
        }
        Some(id) => {
            if id <= 0 {
                return Err(format!("SEC_INVALID_INPUT: invalid oauth account id={id}").into());
...
"#,
    ...
    #[test]
    fn update_tokens_rotates_refresh_token_when_provided() {
        let conn = setup_conn();
        let created = upsert(
            &conn,
            None,
        ...
        .expect("update token with rotated refresh token");

        let updated = get_by_id(&conn, created.id).expect("read updated oauth account");
        assert_eq!(updated.access_token, "token-new");
    ...
    #[test]
    fn record_refresh_failure_keeps_status_and_sets_error_context() {
        let conn = setup_conn();
        let created = upsert(
            &conn,
            None,
        ...
        record_refresh_failure(&conn, created.id, Some("temporary timeout"))
            .expect("record refresh failure");

        let updated = get_by_id(&conn, created.id).expect("read updated account");
        assert_eq!(updated.status, OAuthAccountStatus::Active);
        assert_eq!(updated.last_error.as_deref(), Some("temporary timeout"));
        assert!(updated.last_refreshed_at.is_some());
    }
    ...
    fn quota_mark_clear_and_active_filter() {
        ...
        mark_status(&conn, b.id, OAuthAccountStatus::Disabled, None).expect("disable b");

        let active = list_active_for_cli(&conn, "gemini").expect("list active");
        ...
        mark_status(&conn, b.id, OAuthAccountStatus::Active, None).expect("enable b");
        let active = list_active_for_cli(&conn, "gemini").expect("list active again");
        assert_eq!(active.len(), 2);
    }

    #[test]
    fn list_refresh_and_expired_quota_filters() {
        let conn = setup_conn();
        let error_due = upsert(
            &conn,
            None,
            "claude",
            "ErrorDue",
        ...
        .expect("create error due account");
        let soon = upsert(
            &conn,
            None,
            "claude",
        ...
        .expect("create later account");

        mark_quota_exceeded(&conn, later.id, 900).expect("mark later quota");

        let needing = list_needing_refresh(&conn, 800, 10).expect("list needing refresh");
        assert_eq!(needing.len(), 2);
        assert_eq!(needing[0].id, error_due.id);
        assert_eq!(needing[1].id, soon.id);

        let expired_quotas = list_expired_quotas(&conn, 950, 10).expect("expired quotas");
        assert_eq!(expired_quotas, vec![later.id]);
    }
}
