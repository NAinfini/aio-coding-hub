use crate::{
    circuit_breaker, providers, request_attempt_logs, request_logs, session_manager, settings,
    usage,
};
use axum::{
...
const RECENT_TRACE_DEDUP_TTL_SECS: i64 = 10;
const MAX_NON_SSE_BODY_BYTES: usize = 20 * 1024 * 1024;
pub(super) const LOG_ENQUEUE_MAX_WAIT: Duration = Duration::from_millis(100);

#[derive(Debug, Clone)]
struct CachedGatewayError {
    trace_id: String,
    status: StatusCode,
    error_code: &'static str,
    message: String,
...
pub(super) struct RecentErrorCache {
...
impl RecentErrorCache {
    ...
    fn insert_error(&mut self, now_unix: i64, fingerprint_key: u64, entry: CachedGatewayError) {
        self.prune_expired(now_unix);
...
fn request_log_insert_from_args(
    args: RequestLogEnqueueArgs,
) -> Option<request_logs::RequestLogInsert> {
    let RequestLogEnqueueArgs {
        trace_id,
        cli_key,
        session_id,
        method,
...
pub(super) fn spawn_enqueue_request_log_with_backpressure(
    app: tauri::AppHandle,
    log_tx: tokio::sync::mpsc::Sender<request_logs::RequestLogInsert>,
    args: RequestLogEnqueueArgs,
) {
