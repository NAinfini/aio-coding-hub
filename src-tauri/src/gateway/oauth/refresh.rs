...
use crate::gateway::oauth::token_exchange::{
    refresh_access_token, resolve_effective_access_token, OAuthTokenSet, TokenRefreshRequest,
};
...
async fn refresh_due_accounts(
    ...
    for account in accounts {
        ...
        {
            Ok(tokens) => {
                let (effective_access_token, id_token_to_store) = resolve_effective_access_token(
                    &account.cli_key,
                    &tokens,
