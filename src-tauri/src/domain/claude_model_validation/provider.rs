...
pub(super) async fn load_provider(
    db: db::Db,
    provider_id: i64,
) -> crate::shared::error::AppResult<super::ProviderForValidation> {
    blocking::run(
        "claude_provider_validate_model_load_provider",
        move || -> crate::shared::error::AppResult<super::ProviderForValidation> {
            if provider_id <= 0 {
